...
class ComponentAccessToken {
   ...
   */
  isValid() {
    return !!this.componentAccessToken && Date.now() < this.expireTime;
  }
}
...
class API {
   ...
   */
  constructor(componentAppid, componentAppSecret, authorizerAppid, authorizerRefreshToken, componentVerifyTicket, getComponentToken, saveComponentToken, getToken, saveToken, tokenFromCustom) {
  ...
  async ensureComponentToken() {
    ...
    var accessToken;
    if (component_access_token && (accessToken = new ComponentAccessToken(component_access_token.accessToken, component_access_token.expireTime)).isValid()) {
      return accessToken;
    }
